...
def harvest_asset(asset_id: str,
    ...
    try:
        ...
        # 3b. Trim leading rows donde GDELT aún no arrancó (join tardío)
        #     Estas filas tienen NaN estructurales — no son datos corruptos.
        #     Se eliminan para no contaminar warm-up del LSTM (R11).
        GDELT_KEY_COLS = [
            "entropy_shannon", "goldstein_geo",
            "n_events_ohlcv", "vitality_tesla", "nash_frozen_7d",
        ]
        gdelt_present = [c for c in GDELT_KEY_COLS if c in df_canon.columns]
        if gdelt_present:
            has_all = pl.all_horizontal(
                [pl.col(c).is_not_null() for c in gdelt_present]
            )
            first_valid_idx = (
                df_canon.with_row_index("__idx__")
                .filter(has_all)["__idx__"]
                .min()
            )
            if first_valid_idx is not None and first_valid_idx > 0:
                n_trimmed = int(first_valid_idx)
                df_canon  = df_canon.slice(n_trimmed)
                info(f"  {asset_id}: {n_trimmed} filas GDELT-null trimadas al inicio "
                     f"→ {len(df_canon)} filas útiles")
