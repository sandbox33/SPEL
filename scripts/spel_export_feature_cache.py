...
def export_feature_snapshots(reg: dict, meta: dict) -> dict[str, str]:
    ...
    for asset, lookback in LOOKBACKS.items():
        ...
        # Load parquet — only TENSOR_COLS
        df = pl.scan_parquet(str(pq_path)).select(TENSOR_COLS).collect()

        # Extract tail rows
        n_rows = lookback + 5  # buffer for stride-1 windowing
        ...
        scaler_std = np.where(scaler_std < 1e-8, 1e-8, scaler_std)
        ...
        payload = {
            ...
            "rows":         tail.tolist(),  # [[f32,...×20],...×n_rows]
